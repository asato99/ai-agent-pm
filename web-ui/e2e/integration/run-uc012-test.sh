#!/bin/bash
# UC012 Web UI Integration Test - Send Message from Task Session
# タスクセッションからのメッセージ送信テスト
#
...
# テスト設定
TEST_DB_PATH="/tmp/AIAgentPM_UC012_WebUI.db"
MCP_SOCKET_PATH="/tmp/aiagentpm_uc012_webui.sock"
REST_PORT="8086"
WEB_UI_PORT="5173"  # Must be 5173 for CORS (allowed origins in REST server)

export MCP_COORDINATOR_TOKEN="test_coordinator_token_uc012_webui"
...
cleanup() {
    ...
    if [ "$TEST_PASSED" == "true" ]; then
        rm -f /tmp/uc012_webui_*.log
        rm -f /tmp/coordinator_uc012_webui_config.yaml
        rm -rf /tmp/coordinator_logs_uc012_webui
        rm -f "$TEST_DB_PATH" "$TEST_DB_PATH-shm" "$TEST_DB_PATH-wal"
        rm -rf /tmp/uc012
    else
        echo "Logs preserved: /tmp/uc012_webui_*.log"
    fi
}
...
echo "=========================================="
echo -e "${BLUE}UC012 Web UI Integration Test${NC}"
echo -e "${BLUE}(Send Message from Task Session)${NC}"
echo "=========================================="
echo ""
...
mkdir -p /tmp/uc012
mkdir -p /tmp/uc012/.ai-pm/agents/uc012-worker
mkdir -p /tmp/uc012/.ai-pm/agents/uc012-human
echo "DB: $TEST_DB_PATH"
echo "Working directory: /tmp/uc012"
echo ""
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/uc012_webui_mcp_init.log 2>&1 &
INIT_PID=$!
sleep 3
kill "$INIT_PID" 2>/dev/null || true
rm -f "$MCP_SOCKET_PATH"

SQL_FILE="$SCRIPT_DIR/setup/seed-uc012-data.sql"
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/uc012_webui_mcp.log 2>&1 &
MCP_PID=$!
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" AIAGENTPM_WEBSERVER_PORT="$REST_PORT" \
    "$REST_SERVER_BIN" > /tmp/uc012_webui_rest.log 2>&1 &
REST_PID=$!
...
cat > /tmp/coordinator_uc012_webui_config.yaml << EOF
polling_interval: 2
max_concurrent: 1
...
log_directory: /tmp/coordinator_logs_uc012_webui
EOF

mkdir -p /tmp/coordinator_logs_uc012_webui
$PYTHON -m aiagent_runner --coordinator -c /tmp/coordinator_uc012_webui_config.yaml -v > /tmp/uc012_webui_coordinator.log 2>&1 &
COORDINATOR_PID=$!
sleep 2
...
AIAGENTPM_WEBSERVER_PORT="$REST_PORT" npm run dev -- --port "$WEB_UI_PORT" > /tmp/uc012_webui_vite.log 2>&1 &
WEB_UI_PID=$!
...
npx playwright test \
    ...
    2>&1 | tee /tmp/uc012_webui_playwright.log | grep -E "(✓|✗|passed|failed|UC012|Message)" || true
...
echo "=== Tasks ==="
sqlite3 "$TEST_DB_PATH" "SELECT id, title, status FROM tasks WHERE id LIKE 'uc012-%';"
echo ""
echo "=== Chat Files ==="
echo "Worker chat file:"
cat /tmp/uc012/.ai-pm/agents/uc012-worker/chat.jsonl 2>/dev/null || echo "(empty)"
echo ""
echo "Human chat file:"
cat /tmp/uc012/.ai-pm/agents/uc012-human/chat.jsonl 2>/dev/null || echo "(empty)"
echo ""

# 結果判定
if grep -qE "[0-9]+ passed" /tmp/uc012_webui_playwright.log && ! grep -qE "[0-9]+ failed" /tmp/uc012_webui_playwright.log; then
    TEST_PASSED=true
    echo -e "${GREEN}UC012 Web UI Integration Test: PASSED${NC}"
    exit 0
else
    echo -e "${RED}UC012 Web UI Integration Test: FAILED${NC}"
    echo "Logs: /tmp/uc012_webui_*.log"
    exit 1
fi
