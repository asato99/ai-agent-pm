#!/bin/bash
# Session Status Integration Test
# セッションステータス遷移テスト
#
# フロー:
#   1. テスト環境準備
...
# テスト設定
TEST_DB_PATH="/tmp/AIAgentPM_Session_WebUI.db"
MCP_SOCKET_PATH="/tmp/aiagentpm_session_webui.sock"
REST_PORT="8090"
WEB_UI_PORT="5173"  # Must be 5173 for CORS

export MCP_COORDINATOR_TOKEN="test_coordinator_token_session_webui"
...
cleanup() {
    ...
    if [ "$TEST_PASSED" == "true" ]; then
        rm -f /tmp/session_webui_*.log
        rm -f /tmp/coordinator_session_webui_config.yaml
        rm -rf /tmp/coordinator_logs_session_webui
        rm -f "$TEST_DB_PATH" "$TEST_DB_PATH-shm" "$TEST_DB_PATH-wal"
        rm -rf /tmp/session-test
    else
        echo "Logs preserved: /tmp/session_webui_*.log"
    fi
}
...
echo "=========================================="
echo -e "${BLUE}Session Status Integration Test${NC}"
echo "=========================================="
echo ""
...
mkdir -p /tmp/session-test/.ai-pm/agents/session-worker
mkdir -p /tmp/session-test/.ai-pm/agents/session-human
echo "DB: $TEST_DB_PATH"
echo "Working directory: /tmp/session-test"
echo ""
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/session_webui_mcp_init.log 2>&1 &
INIT_PID=$!
sleep 3
...
# Load test data
SQL_FILE="$SCRIPT_DIR/setup/seed-session-data.sql"
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/session_webui_mcp.log 2>&1 &
MCP_PID=$!
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" AIAGENTPM_WEBSERVER_PORT="$REST_PORT" \
    "$REST_SERVER_BIN" > /tmp/session_webui_rest.log 2>&1 &
REST_PID=$!
...
cat > /tmp/coordinator_session_webui_config.yaml << EOF
polling_interval: 2
max_concurrent: 1
...
log_directory: /tmp/coordinator_logs_session_webui
EOF

mkdir -p /tmp/coordinator_logs_session_webui
$PYTHON -m aiagent_runner --coordinator -c /tmp/coordinator_session_webui_config.yaml -v > /tmp/session_webui_coordinator.log 2>&1 &
COORDINATOR_PID=$!
sleep 2
...
AIAGENTPM_WEBSERVER_PORT="$REST_PORT" npm run dev -- --port "$WEB_UI_PORT" --strictPort > /tmp/session_webui_vite.log 2>&1 &
WEB_UI_PID=$!
...
npx playwright test \
    ...
    session-status.spec.ts \
    2>&1 | tee /tmp/session_webui_playwright.log | grep -E "(✓|✗|passed|failed|Session|connected|reconnect|status)" || true
...
echo "=== Agent Sessions ==="
sqlite3 "$TEST_DB_PATH" "SELECT id, agent_id, project_id, purpose, state, expires_at FROM agent_sessions WHERE agent_id LIKE 'session-%';" 2>/dev/null || echo "(no sessions)"
echo ""
echo "=== Pending Agent Purposes ==="
sqlite3 "$TEST_DB_PATH" "SELECT agent_id, project_id, purpose, started_at FROM pending_agent_purposes WHERE agent_id LIKE 'session-%';" 2>/dev/null || echo "(none)"
echo ""

# 結果判定
if grep -qE "[0-9]+ passed" /tmp/session_webui_playwright.log && ! grep -qE "[0-9]+ failed" /tmp/session_webui_playwright.log; then
    TEST_PASSED=true
    echo -e "${GREEN}Session Status Integration Test: PASSED${NC}"
    exit 0
else
    echo -e "${RED}Session Status Integration Test: FAILED${NC}"
    echo "Logs: /tmp/session_webui_*.log"
    exit 1
fi
