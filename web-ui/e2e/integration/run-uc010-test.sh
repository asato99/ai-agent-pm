...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/uc010_webui_mcp_init.log 2>&1 &
INIT_PID=$!
sleep 3
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" AIAGENTPM_WEBSERVER_PORT="$REST_PORT" \
    "$REST_SERVER_BIN" > /tmp/uc010_webui_rest.log 2>&1 &
REST_PID=$!
...
if grep -qE "[0-9]+ passed" /tmp/uc010_webui_playwright.log && ! grep -qE "[0-9]+ failed" /tmp/uc010_webui_playwright.log; then
    TEST_PASSED=true
