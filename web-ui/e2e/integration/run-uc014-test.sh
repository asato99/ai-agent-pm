#!/bin/bash
# UC014 Web UI Integration Test - Chat Session Immediate Response
# チャットセッション即時応答テスト
#
...
# テスト設定
TEST_DB_PATH="/tmp/AIAgentPM_UC014_WebUI.db"
MCP_SOCKET_PATH="/tmp/aiagentpm_uc014_webui.sock"
REST_PORT="8087"
WEB_UI_PORT="5173"  # Must be 5173 for CORS (allowed origins in REST server)

export MCP_COORDINATOR_TOKEN="test_coordinator_token_uc014_webui"
...
cleanup() {
    ...
    if [ "$TEST_PASSED" == "true" ]; then
        rm -f /tmp/uc014_webui_*.log
        rm -f /tmp/coordinator_uc014_webui_config.yaml
        rm -rf /tmp/coordinator_logs_uc014_webui
        rm -f "$TEST_DB_PATH" "$TEST_DB_PATH-shm" "$TEST_DB_PATH-wal"
        rm -rf /tmp/uc014
    else
        echo "Logs preserved: /tmp/uc014_webui_*.log"
    fi
}
...
echo "=========================================="
echo -e "${BLUE}UC014 Web UI Integration Test${NC}"
echo -e "${BLUE}(Chat Session Immediate Response)${NC}"
echo "=========================================="
echo ""
...
mkdir -p /tmp/uc014
mkdir -p /tmp/uc014/.ai-pm/agents/uc014-worker
mkdir -p /tmp/uc014/.ai-pm/agents/uc014-human
echo "DB: $TEST_DB_PATH"
echo "Working directory: /tmp/uc014"
echo ""
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/uc014_webui_mcp_init.log 2>&1 &
INIT_PID=$!
sleep 3
kill "$INIT_PID" 2>/dev/null || true
rm -f "$MCP_SOCKET_PATH"

SQL_FILE="$SCRIPT_DIR/setup/seed-uc014-data.sql"
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" "$MCP_SERVER_BIN" daemon \
    --socket-path "$MCP_SOCKET_PATH" --foreground > /tmp/uc014_webui_mcp.log 2>&1 &
MCP_PID=$!
...
AIAGENTPM_DB_PATH="$TEST_DB_PATH" AIAGENTPM_WEBSERVER_PORT="$REST_PORT" \
    "$REST_SERVER_BIN" > /tmp/uc014_webui_rest.log 2>&1 &
REST_PID=$!
...
cat > /tmp/coordinator_uc014_webui_config.yaml << EOF
polling_interval: 2
max_concurrent: 1
...
log_directory: /tmp/coordinator_logs_uc014_webui
EOF

mkdir -p /tmp/coordinator_logs_uc014_webui
$PYTHON -m aiagent_runner --coordinator -c /tmp/coordinator_uc014_webui_config.yaml -v > /tmp/uc014_webui_coordinator.log 2>&1 &
COORDINATOR_PID=$!
sleep 2
...
AIAGENTPM_WEBSERVER_PORT="$REST_PORT" npm run dev -- --port "$WEB_UI_PORT" > /tmp/uc014_webui_vite.log 2>&1 &
WEB_UI_PID=$!
...
npx playwright test \
    ...
    chat-session-immediate-response.spec.ts \
    2>&1 | tee /tmp/uc014_webui_playwright.log | grep -E "(✓|✗|passed|failed|UC014|Response|session)" || true
...
echo "=== Agent Sessions ==="
sqlite3 "$TEST_DB_PATH" "SELECT id, agent_id, project_id, purpose, expires_at FROM agent_sessions WHERE agent_id LIKE 'uc014-%';" 2>/dev/null || echo "(no sessions)"
echo ""
echo "=== Pending Agent Purposes ==="
sqlite3 "$TEST_DB_PATH" "SELECT agent_id, project_id, purpose, created_at, started_at FROM pending_agent_purposes WHERE agent_id LIKE 'uc014-%';" 2>/dev/null || echo "(none)"
echo ""
echo "=== Chat Files ==="
echo "Worker chat file:"
cat /tmp/uc014/.ai-pm/agents/uc014-worker/chat.jsonl 2>/dev/null || echo "(empty)"
echo ""
echo "Human chat file:"
cat /tmp/uc014/.ai-pm/agents/uc014-human/chat.jsonl 2>/dev/null || echo "(empty)"
echo ""

# 結果判定
if grep -qE "[0-9]+ passed" /tmp/uc014_webui_playwright.log && ! grep -qE "[0-9]+ failed" /tmp/uc014_webui_playwright.log; then
    TEST_PASSED=true
    echo -e "${GREEN}UC014 Web UI Integration Test: PASSED${NC}"
    exit 0
else
    echo -e "${RED}UC014 Web UI Integration Test: FAILED${NC}"
    echo "Logs: /tmp/uc014_webui_*.log"
    exit 1
fi
