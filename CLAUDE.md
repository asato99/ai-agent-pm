...
## ⚠️⚠️⚠️ 統合テストにおける絶対禁止事項 ⚠️⚠️⚠️

### 🚫 期待結果のシードは最悪の不正行為

**絶対禁止**: 統合テストのシードデータに**期待結果（テストで検証すべきデータ）を事前に作成してはならない**
...
**なぜ最悪の不正か**:
- 統合テストの本質は「実際のシステムが正しく連携して動作するか」の検証
- 期待結果をシードすると、機能が動作していなくてもテストが通る
- テストの価値がゼロになり、バグを見逃す
- 偽りのGREEN状態を作り出し、チーム全体を欺く行為
...
**原則**: 1機能につき1つの最小限REDテストを作成し、実装後に同じテストがGREENになる
...
**例: タスクステータス遷移制限（Feature 12）**

```swift
// 1つのテストで必要な検証をすべて行う
func testInProgressToTodoTransitionDisabled() throws {
    // 検証1: ステータスピッカーでTo Doオプションが無効化
    // 検証2: ドラッグ＆ドロップでTo Doカラムへの移動が拒否
}
```

- REDフェーズ: 遷移制限が未実装 → To Doオプションが有効 → テスト失敗
- GREENフェーズ: 遷移制限を実装 → To Doオプションが無効化 → 同じテストが成功

**禁止事項**:
- 「許可された遷移のテスト」を別途作成しない
- 「正常系」「異常系」で分割しない（1テストに統合）
- 実装後に新規テストを追加しない（同じテストがGREENになるだけ）
...
テストが失敗した場合、**ログの「成功」メッセージを鵜呑みにせず**、以下の手順で調査する：

1. **error-context.md を最優先で確認**
   ```bash
   cat web-ui/test-results/*/error-context.md
   ```
   - Playwrightが生成するページスナップショットには**UIの実際の状態**が記録されている
   - 表示されているデータが期待値と一致するか確認

2. **表示データと期待データの照合**
   - テストが検証しようとしているデータ
   - 実際にUIに表示されているデータ
   - **両者が異なる場合、残存データの可能性を疑う**

3. **DBの直接確認**
   ```bash
   sqlite3 /tmp/AIAgentPM_Pilot.db "SELECT * FROM chat_messages"
   sqlite3 /tmp/AIAgentPM_Pilot.db "SELECT * FROM tasks"
   ```

4. **シードデータのクリーンアップ漏れを確認**
   - 前回テストのデータが残っていないか
   - 関連する全テーブルがクリアされているか

**パイロットテスト固有**: [`web-ui/e2e/pilot/docs/LOG_STRATEGY.md`](web-ui/e2e/pilot/docs/LOG_STRATEGY.md) 参照
...
### パイロットテスト: シードデータのクリーンアップ漏れ（2026-01-25修正）

**問題**: seed-generator.ts がチャット関連テーブルをクリアしていなかった

**症状**:
- テストログでは「Chat session is ready」「Sent initial message」と成功を示す
- しかしUIには**前回テストの古いメッセージ**が表示されている
- マネージャーが古いリクエストに既に応答済み
- テストの検証が誤ったデータにマッチして通過

**根本原因**: 以下のテーブルがクリアされていなかった
- `chat_messages`
- `chat_sessions`
- `pending_agent_purposes`

**教訓**:
1. **error-context.md は必ず確認** - 実際のUI状態が記録されている
2. **ログの「成功」を信じすぎない** - 別のデータにマッチしている可能性
3. **シードは関連する全テーブルをクリア** - 漏れがあると残存データで誤動作
...
| 2026-01-15 | TDD RED-GREEN原則追加: 1機能1テスト、実装後も同じテストを使用 |
| 2026-01-24 | **⚠️ 統合テスト絶対禁止事項追加**: 期待結果のシードは最悪の不正行為として明記 |
| 2026-01-25 | E2E/統合テスト失敗時の調査手順追加、シードデータクリーンアップ漏れ問題を記録 |
