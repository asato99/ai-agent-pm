...
**注記**: TS-03-009, 011, 012はアプリのUI機能自体が未実装のためテスト不可
...
**実装状態**: ✅ 実装済み
**検証内容**: AgentsSection識別子を持つ要素の存在確認
...
**実装状態**: ✅ 実装済み
**検証内容**: テストデータで作成されたエージェント（owner, backend-dev）の名前が表示されることを確認
...
**実装状態**: ✅ 実装済み
**検証内容**: キーボードショートカット(⇧⌘A)でシートが表示されることを検証
...
**実装状態**: ✅ 実装済み
**検証内容**: ステータスアイコン（🟢/🟡/🟠）のいずれかが表示されることを確認
...
**実装状態**: ✅ 実装済み
**検証内容**: エージェント選択後にAgentDetailView識別子の詳細パネルと統計セクション（Statistics）の表示確認
...
**実装状態**: ✅ 実装済み
**検証内容**: AgentNameFieldとAgentRoleFieldの存在確認
**テストメソッド**: `testAgentCreationFormBasicInfo()`
...
**実装状態**: ✅ 実装済み
**検証内容**: Typeセクション、Role Type/Agent Typeラベル、AI/Humanオプションの存在確認
**テストメソッド**: `testAgentCreationFormTypeSelection()`
...
**実装状態**: ⏳ 未実装（UI未実装）
**備考**: 3ステップウィザード形式は未実装（現在はシンプルなフォーム形式）。テストはXCTFailで明示的に失敗。
**テストメソッド**: `testAgentCreationWizardStep3()`
...
**実装状態**: ✅ 実装済み
**検証内容**: エージェント選択後に統計セクション（Statistics）が表示されることを確認
**テストメソッド**: `testAgentStatsSection()`
...
**実装状態**: ⏳ 未実装（UI未実装）
**備考**: エージェント活動履歴タブは未実装。テストはXCTFailで明示的に失敗。
**テストメソッド**: `testAgentActivityHistoryTab()`
...
**実装状態**: ⏳ 未実装（UI未実装）
**備考**: エージェントコンテキストメニューは未実装。テストはXCTFailで明示的に失敗。
**テストメソッド**: `testAgentContextMenu()`
...
**実装状態**: ✅ 実装済み（部分的）
**検証内容**: 詳細パネルが表示され編集可能な状態であることを確認（階層タブの詳細検証は未実装）
**テストメソッド**: `testAgentEditButton()`
...
**実装状態**: ✅ 実装済み
**検証内容**: Hierarchy & Resourcesセクション、Parent Agentラベル、None (Top Level)オプションの存在確認
**テストメソッド**: `testAgentFormParentAgentPicker()`
...
**実装状態**: ✅ 実装済み
**検証内容**: Max Parallel Tasksラベルと設定用Stepperの存在確認
**テストメソッド**: `testAgentFormMaxParallelTasks()`
...
| 2025-01-06 | ドキュメントを実装状態に合わせて修正。進捗サマリ更新（✅12件、⏳3件）。テストメソッド名更新。 |
