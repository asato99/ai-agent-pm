...
**注記**: タブ形式UI(TS-04-002)、サブタスク(別途)、履歴フィルター(TS-04-009)はアプリのUI機能が未実装
...
**実装状態**: ✅ 実装済み
**検証内容**: タスク詳細ビュー（TaskDetailView）、Detailsセクション、ステータスバッジ（Backlog/To Do/In Progress/Done/Blocked/Cancelled）、優先度バッジ（Urgent/High/Medium/Low）の存在確認
...
**実装状態**: ⏳ 未実装（UI未実装）
**備考**: アプリのタブ形式UIが未実装（現在はスクロールビュー形式）。テストはXCTFailで明示的に失敗。
...
**実装状態**: ✅ 実装済み
**検証内容**: キーボードショートカット（⇧⌘H）でハンドオフシートが表示されることを確認（macOS SwiftUIのツールバーボタンはXCUITestに公開されないため代替手法）
...
**実装状態**: ✅ 実装済み
**検証内容**: キーボードショートカット（⌘E）で編集シートが表示されることを確認（macOS SwiftUIのツールバーボタンはXCUITestに公開されないため代替手法）
...
**実装状態**: ✅ 実装済み
**検証内容**: Detailsセクション、Statusラベル、ステータスピッカー（popUpButton）の存在確認。ステータスオプション（Backlog/To Do/In Progress/Done/Blocked/Cancelled）の確認
**テストメソッド**: `testStatusChangePicker()`
...
**実装状態**: ⏳ 未実装（UI未実装）
**備考**: アプリの履歴フィルター機能が未実装。テストはXCTFailで明示的に失敗。
**テストメソッド**: `testHistoryFilter()`
...
**テストメソッド**: `testContextListDisplay()`
...
**テストメソッド**: `testHandoffListDisplay()`
...
| 2025-01-06 | ドキュメントを実装状態に合わせて修正。10件が実装済み、3件（タブ/サブタスク/履歴フィルター）は未実装。テストメソッド名と検証内容を追加。 |
