...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。各カラム（Backlog, To Do, In Progress等）のテキスト存在確認が必要。
**テストメソッド**: `testKanbanColumnsStructure()`
...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。ステータス名と件数表示の検証なし。
**テストメソッド**: `testColumnHeadersShowStatusAndCount()`
...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。「+ タスク」ボタンの存在確認とタップ動作の検証なし。
...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。タスクカード要素（タイトル、優先度、担当者）の検証なし。
...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。タスク選択と詳細パネル表示の検証なし。
...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。優先度バッジの色分け検証なし。
**テストメソッド**: `testPriorityBadgeColors()`
...
**実装状態**: 🔸 スタブ
**現状**: `assertWindowExists()` のみ。リフレッシュボタンの存在確認なし。
